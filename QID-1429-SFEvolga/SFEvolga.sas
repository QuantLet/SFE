...
* Quantlet:     SFEvolga  
* ---------------------------------------------------------------------
* Description:  SFEvolga plots the volga of a call option
* ---------------------------------------------------------------------
* Usage:        SFEvolga  
* ---------------------------------------------------------------------
...
* ---------------------------------------------------------------------
* Keywords:     Black Scholes, call, financial, greeks, option, volga
* -------------------------------------------------------------------
* Output:       plot of the volga of a call option
* ---------------------------------------------------------------------
* Example:      User inputs [lower, upper] bound of Asset price S like
*               [50,150], [lower, upper] bound of time to maturity tau 
*               like [0.05, 1], then plot of the volga of a call option 
*               is given.
* ---------------------------------------------------------------------
...
Volga=(S#sqrt(tau)#exp(-q#tau)#pdf('Normal',d1)#d1#(d1-sig#sqrt(tau)));
...
data d3d;set d3d;
rename col1 = S col2 = volga col3 = tau;

*Plot the 3d surface graph;

 title h = 2 f = default 'Volga as function of the time to maturity ' 
h = 3 f = greek '74'x h = 2 f = default ' and the asset price S';


proc g3d data = d3d ;
 plot S*tau = volga/  cbottom = red ctop = blue
grid  rotate=45;
label  S='Asset price'
...
run;
quit;
*Open the file 'd:volga.gif' and enjoy the 3D animation;
